...
В нашил проектах мы используем [react-hook-form](https://www.react-hook-form.com) для валидации форм
...
Для начала напишем интерфейс для данных, который будет отправлять наша форма. Я буду делать форму регистрации
...
Для того, чтобы начать использовать библиоетеку, вызовем в компоненте с **формой** хук `useForm` и в его дженерики напишем наш интефейс `IForm`
...
Перед тем как продолжить писать форму, напишем компонент Input
...
Для того, чтобы сделать поле обязательным используем функци `validateRequired`
